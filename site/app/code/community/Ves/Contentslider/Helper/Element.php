...
class Ves_Contentslider_Helper_Element extends Mage_Core_Helper_Abstract {
    ...
    public function getElementStores($name, $element_name, $value = array(), $attr = ""){
    	...
    	if(empty($value) || in_array(0,$value)){
    		$html .= '<option value="0" selected="selected">'.Mage::helper("ves_contentslider")->__("All Store Views").'</option>';
    	}else{
    		$html .= '<option value="0">'.Mage::helper("ves_contentslider")->__("All Store Views").'</option>';
    	}
    ...
    public function getElementEditor($textarea_name, $element_name, $content = "", $attr = ""){
        ...
        $html ='<span class="field-row">
                    <div id="buttons'.$textarea_name.'" class="buttons-set">
                        <button type="button" class="scalable show-hide" style="" id="toggle'.$textarea_name.'"><span><span><span>'.Mage::helper('ves_contentslider')->__('Show / Hide Editor').'</span></span></span></button>
                        <button type="button" class="scalable add-widget plugin" onclick="widgetTools.openDialog(\''.$this->getWidgetLink(array('widget_target_id'=>$textarea_name)).'\')" style="display:none"><span><span><span>'.Mage::helper('ves_contentslider')->__('Insert Widget...').'</span></span></span></button>
                        <button type="button" class="scalable add-image plugin" onclick="MediabrowserUtility.openDialog(
                        \''.$this->getImagesLink(array('target_element_id'=>$textarea_name)).'\')" style="display:none"><span><span><span>'.Mage::helper('ves_contentslider')->__('Insert Image...').'</span></span></span></button>

                        <button type="button" class="scalable add-variable plugin" onclick="MagentovariablePlugin.loadChooser(\''.$this->getVariablesLink().'\', \''.$textarea_name.'\');" style="display:none;"><span><span><span>'.Mage::helper('ves_contentslider')->__('Insert Variable...').'</span></span></span></button></div>
